...
int main(void)
{
	...
	// HDL monitoring area device
	char mon_command_area_code[] = {0x00, 0x02};
	char mon_device_area_id[] = {0x5A}; // 90
	char mon_channel_area_id[] = {0x01};
	char mon_area_val[] = {0x00};
	...
	while (1) {
		...
		if (FD_ISSET(sockfd, &read_flags)) {
			...
			if ((numbytes = recv(sockfd, buf, MAXBUFLEN-1, 0)) > 0) {
				...
				if (memcmp(&command_code, &mon_command_area_code, 2) == 0) {
					if (memcmp(&device_id, &mon_device_area_id, 1) == 0) {
						if (memcmp(&channel_id, &mon_channel_area_id, 1) == 0) {
